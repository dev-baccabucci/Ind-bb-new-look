    ...
    section.querySelectorAll(".js-media-list").forEach((elem, index) => {
      ...
      let slider = new Swiper(elem, {
        ...
        scrollbar: {
           ...
           breakpoints: {
        0: { // for mobile
        slidesPerView: 1.1, // 1 full + bit of next
        spaceBetween: 10,
        },
        990: {
          speed: 750,
...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    // Clear error message when user selects options
    const productForm = this.closest('.product')?.querySelector('product-form') || document.querySelector('product-form');
    if (productForm && typeof productForm.handleErrorMessage === 'function') {
      productForm.handleErrorMessage(false);
    }

    // Update media if variant is selected
  ...
  updateMasterId() {
    if (this.variantData || this.querySelector('[type="application/json"]')) {
      // If any option is unselected (null or blank), do NOT auto-assign a variant
  ...
  updateVariantStatuses() {
    const checkedOption1 = this.querySelector('fieldset[id="SingleOptionSelector-0"] :checked');
    if (!checkedOption1) return;

    const selectedOptionOneVariants = this.variantData.filter(
      (variant) => checkedOption1.value === variant.option1
    );
  ...
  toggleAddButton(disable = true, text) {
    ...
    if (disable) {
      ...
      if (addButtonText) addButtonText.textContent = window.variantStrings?.addToCart || "Add to cart";
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fallbackMsg = document.getElementById('shareFallbackMsg');

  if (!shareBtn) return;

  shareBtn.addEventListener('click', async () => {
    const productUrl = shareBtn.getAttribute('data-url');
    const productTitle = shareBtn.getAttribute('data-title');
    ...
    if (navigator.share) {
      try {
        ...
        console.log('Product shared successfully via native UI');
      } catch (err) {
        // User cancelled the share or an error occurred
        console.log('Sharing cancelled or failed:', err);
      }
    } else {
      // Fallback: Copy to Clipboard for unsupported browsers (mostly older desktops)
      try {
        await navigator.clipboard.writeText(productUrl);
        
        // Show success message
        fallbackMsg.classList.add('is-visible');
        
        // Hide message after 3 seconds
        setTimeout(() => {
          fallbackMsg.classList.remove('is-visible');
        }, 3000);
      } catch (err) {
